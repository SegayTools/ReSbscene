...
| `LAYR` | `0x03` / `0x20` / `0x21` / `0x22` | Confirmed(raw)/Unknown semantics | 图层名、shared packed state raw word、两个 UInt16 raw count-like 字段。full survey 中字段顺序固定；`0x20` 只见 `0/0x100`，`0x21` 按 scene 求和等于 `NODE` 总数，`0x22` 与 `ParamLow` 只有一个 `+2` 例外。运行时用途未确认。 |
...
| `CIMG` | `0x44` / `0x45` | Confirmed(raw)/Unknown high-level semantics | `0x44` 是 primary/secondary 两组 CREF 记录数；full survey 中 7063/7063 与 7228/7228 个 image cast 与后续 CREF 组完全匹配。`0x45` 为两个组内 crop reference index，且已核对的范围检查逻辑按 `0x44` 两组 count 对其做范围校验。新增 `ImageVariantGroupCimg45FirstKey*` 显示 `0x45` 与 type 18/19 最早 key 经常一致但不是全覆盖；是否命名为默认、当前或选中引用仍需渲染/播放代码确认。 |
...
| `ANIM` | `0x03` / `0x50` / `0x56` / `0x5F` | Confirmed(raw)/Candidate semantics | 动画名 / declared motion count candidate / declared nominal end frame candidate / Byte/Bool-like raw flag。`0x50` 与实际 motion 数匹配 1,578/1,613 个动画，但 full survey 有 1 个 `+2` 异常；该异常虽等于本动画最大 motion track 数，新增全量交叉显示这不是全局规则，因此不写成 confirmed count。`0x56` 常等于最大 track/key frame，但 JP/EN 分别有 47/47 个动画小于最大 track last，91/93 个动画大于最大 track last，因此不写成所有 track/key 的严格上界。`0x5F` 只见 `0/1`，但不是 motion-presence flag、loop flag 或 count modifier，运行时语义未知。 |
| `MOT ` | `0x51` / `0x52` | Confirmed(raw)/Candidate target | `0x52` 为 track 数量；full survey 中与 `MOT.ParamLow` 和实际 track 数在 JP/EN 32,428/32,742 个 motion 中全量一致。`0x51` 在当前 full survey 中落在 NODE index 范围内，Ras 可映射到 `NODE` 记录 index；运行时 target 角色仍按候选绑定处理。 |
...
`ImageVariantGroupCimg45FirstKey*` 进一步把 `CIMG.0x45` 与 type 18/19 image variant track 的最早 key 做静态比较。JP 中 `18 primary` 为 match 1533 / mismatch 258，`19 secondary` 为 match 332 / mismatch 1；EN 中 `18 primary` 为 match 1534 / mismatch 257 / multi-CIMG target 1，`19 secondary` 为 match 332 / mismatch 1。`firstKey - cimg45` 的差值在 primary 中覆盖负值、0 和正值，secondary 只有 `0` 与 `-1`。因此 `0x45` 可与很多变体轨道起始值对齐，但不能全局写成动画初始 key、默认引用、当前引用或选中引用；这些更高层命名仍待渲染或播放代码确认。
...
type 18 / 19 当前分别命名为 `PrimaryImageVariantIndexCandidate` / `SecondaryImageVariantIndexCandidate`。full survey 中 JP/EN 分别有 1,791/1,792 条 type 18 track、6,986/6,988 个 key；legacy `ImageVariant*` 字段继续保留 type 18 的 primary+secondary 合计数量宽松检查，仅用于兼容早期统计。新增 `ImageVariantGroup*` 聚合按 primary/secondary CREF 组分别复核：`18 primary` 在 JP/EN 为 1,791/1,792 条 track、6,986/6,988 个 key，全部目标节点有 CIMG、全部 track/key in range；`19 secondary` 在 JP/EN 均为 333 条 track、9,769 个 key，同样全部有 CIMG 且全部 in range。type 19 secondary 的 CREF count 分布为 `2:41, 5:16, 32:276`，key value 分布为 0..31。`ImageVariantGroupCimg45FirstKey*` 显示 type 18/19 最早 key 与 `CIMG.0x45` 的对应关系不是全覆盖：primary JP/EN 为 1533/1534 match、258/257 mismatch，secondary 两组都是 332 match、1 mismatch。`sub_7CE8B0` 对 type 18 使用 CIMG primary CREF count 校验，对 type 19 使用 secondary CREF count 校验；因此 type 18 不应命名为跨 primary/secondary 两组的统一选择，`CIMG.0x45` 也不能全局命名为该轨道的起始 key。播放时如何与 `CIMG.0x45` 和渲染状态组合仍需确认。
